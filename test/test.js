const FrajBox = artifacts.require('./FrajBox.sol')

require('chai')
  .use(require('chai-as-promised'))
  .should()

contract('FrajBox', ([deployer, uploader]) => {
  let frajbox

  before(async () => {
    frajbox = await FrajBox.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await frajbox.address
    ...
    it('has a name', async () => {
      const name = await frajbox.name()
      assert.equal(name, 'FrajBox')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await frajbox.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await frajbox.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await frajbox.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await frajbox.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await frajbox.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await frajbox.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await frajbox.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await frajbox.files(fileCount)
